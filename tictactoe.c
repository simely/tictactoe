...
int oyunBaslat() {
	...
	do {
		...
		oyuncuSira = (oyuncuSira % 2) ? 1 : 2;  // oyuncu değişkeninin değerinin 2 ile bölümünden kalan varsa oyuncu = 1 değerini alır, yoksa oyuncu = 2 değerini alır.
	    ...
	    if(secim == 1 && kutu[1] == '1')
			...
			kutu[9] = isaret;
			   ...
			   oyuncuSira--;  // Hatalı seçim yapıldığında aynı oyuncunun tekrar oynayabilmesi için oyuncu değişkeni 1 azaltılır.
		}
		
		i = kazananKontrol();
		
		oyuncuSira++;  // Sıradaki oyuncuya geçmek için oyuncu değişkeni 1 arttırılır.
}
